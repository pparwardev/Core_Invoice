#!/bin/bash
# Final build: Core-Invoice Portable Package with MCP PO Reader
echo "🧾 Core-Invoice Final Portable Build (with MCP)"
echo "================================================="
...
echo ""
echo "📦 Step 4: Copying MCP PO Reader (Python scripts)..."
...
echo  =============================================
echo.
echo  This installs Python dependencies for
echo  advanced PO PDF extraction (pdfplumber).
echo.
echo  Note: Basic PO reading works without Python.
echo  This setup enables FULL extraction accuracy.
echo.
...
echo.
echo  ✓ MCP PO Reader setup complete!
echo  Advanced PDF extraction is now available.
echo.
pause
BATCH
...
HOW TO RUN:
  ...
  4. Register a new account and start using!

PO READER (MCP) SETUP:
  For full PO PDF extraction accuracy:
  1. Install Python 3.10+ (https://python.org)
  2. Run "Setup-Python-MCP.bat"
  3. PO Reader will use advanced extraction

  Note: Basic PO reading works without Python!
  Python adds pdfplumber for better table/layout parsing.

FEATURES:
  * Animated Splash Screen
  * User Registration (strong password, all fields)
  * Login/Logout with session management
  * Dashboard with analytics
  * Vendor Management (CRUD)
  * Tax Invoice Generation
  * PO Reader - PDF upload & auto-extract
    - Purchase Order number, dates, amounts
    - Vendor GSTIN, PAN, MSME
    - Line items with HSN/SAC codes
    - Auto-maps to vendors
...
rm -f ~/Core_invoice/Core-Invoice-v1.0.0-Windows.zip
zip -r ~/Core_invoice/Core-Invoice-v1.0.0-Windows.zip Core-Invoice/ -q
ZIP_SIZE=$(du -sh ~/Core_invoice/Core-Invoice-v1.0.0-Windows.zip | cut -f1)
echo "✓ Zip created: Core-Invoice-v1.0.0-Windows.zip ($ZIP_SIZE)"
echo ""
echo "Done! Distribute the zip to any Windows PC."
